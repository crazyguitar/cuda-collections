...
struct Test {
  int input[BUFSIZE];
  int host_ptr[BUFSIZE];
  int* dev_ptr;
  cudaStream_t stream;

  __host__ Test() {
    CUDA_CHECK(cudaSetDevice(0));
    // zoer-copy. map host memory to GPU memory. We can also use cudaMallocHost.
    CUDA_CHECK(cudaHostRegister(host_ptr, sizeof(int) * BUFSIZE, cudaHostRegisterDefault));
